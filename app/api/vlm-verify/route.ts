...
export async function POST(request: NextRequest) {
  try {
    ...
    const vlmApiKey = process.env.VLM_API_KEY || process.env.NIM_API_KEY;
    const vlmModel = process.env.VLM_MODEL || "nvidia/cosmos3-nano-reasoner";

    if (!vlmApiUrl || !vlmApiKey || (!body.imageUrl && !body.videoUrl)) {
            ...
            content: [
              {
                ...
                text:
                  ...
                  "Review the provided visual input and give a short verification summary in 1-2 sentences.",
              },
              {
                type: body.imageUrl ? "image_url" : "video_url",
                [body.imageUrl ? "image_url" : "video_url"]: {
                  url: mediaUrl,
                },
              },
            ],
